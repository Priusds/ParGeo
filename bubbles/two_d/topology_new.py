    ...
    def add(
        ...
        for running_level, running_polygon in self.__topology.items(): 
            ...
            if level != running_level : 
                ...
                if isinstance(lower_polygon, (shapely.MultiPolygon, shapely.GeometryCollection)):
                    lower_polygon = shapely.MultiPolygon([p for p in lower_polygon.geoms if p.area > self.grid_size and isinstance(p, shapely.Polygon)])
                
                # add the intersection points
                intersection = higher_polygon.intersection(lower_polygon, grid_size=self.grid_size)
                lower_polygon = lower_polygon.union(intersection, grid_size=self.grid_size)
        ...
        updated_topo[level] = polygon if not level in self.topology else polygon.union(self.topology[level], grid_size=self.grid_size)
