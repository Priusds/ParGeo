...
class Sphere(Bubble):
    ...
    def discretize(self, accuracy, point_id=1, line_id=1, lineLoop_id=1):
        geometry = discretize_reference(
            self.trafo, accuracy, point_id, line_id, lineLoop_id
        )
    ...
    def getBoundingBox(self):
        return [x + self.radius for x in self.midpoint] + [
            x - self.radius for x in self.midpoint
        ]
...
class Ellipse(Bubble):
    ...
    def trafo(self, theta, phi):
        ...
        return (a * b * c) / np.sqrt(
            c**2
            * (b**2 * np.cos(phi) ** 2 + a**2 * np.sin(phi) ** 2)
            * np.cos(theta) ** 2
            + a**2 * b**2 * np.sin(theta) ** 2
        )
    ...
    def getBoundingBox(self):
        return [x + max(self.radii) for x in self.midpoint] + [
            x - max(self.radii) for x in self.midpoint
        ]
...
class StarBubble(Bubble):
    ...
    def trafo(self, theta, phi):
        ...
        a, b, c = 1, 1, 1
        res[0] = (
            (1 + 0.2 * np.sin(3 * p[0]) + 0.3 * np.cos(5 * p[1]))
            * a
            * np.sin(p[0])
            * np.cos(p[1])
        )  # (1+0.2*np.sin(3*p[0]) + .3*np.cos(5*p[1]))*
        res[1] = (
            b
            * (1 + 0.2 * np.sin(3 * p[0]) + 0.3 * np.cos(5 * p[1]))
            * np.sin(p[0])
    ...
    def getBoundingBox(self, nsample=40):
        ...
        points = [
            _targetPoint(theta, phi, self.trafo(theta, phi))
            for theta, phi in zip(ltheta, lphi)
        ]
...
def clip(geometry, clippingPlane, pointProjection, clippingRule, args=None):
    ...
    for triangle, lines in geometry["lineLoops"].items():
        ...
        points = {geometry["lines"][l][0] for l in lines_id}.union(
            {geometry["lines"][l][1] for l in lines_id}
        )
        points_locations = {
            point: locate(
                geometry["points"][point]["coords"], normalVector, supportVector
            )
            for point in points
        }
    ...
    ltriangle_boundary, lline_boundary, new_points = clipper.apply(
        ltriangle_boundary, lline_boundary, lpoint_boundary, geometry
    )

    lpoint_inside = lpoint_inside.difference(lpoint_boundary)
    lline_inside = lline_inside.difference(lline_boundary)
    clip_geometry = dict()
    clip_geometry["points"] = {
        **{Id: geometry["points"][Id] for Id in lpoint_inside},
        **new_points,
    }
    clip_geometry["lines"] = {
        **{Id: geometry["lines"][Id] for Id in lline_inside},
        **{Id: geometry["lines"][Id] for Id in lline_boundary},
    }
    clip_geometry["lineLoops"] = {
        **{Id: geometry["lineLoops"][Id] for Id in ltriangle_inside},
        **{Id: geometry["lineLoops"][Id] for Id in ltriangle_boundary},
    }
...
def discretize_reference(trafo, accuracy, point_id=1, line_id=1, lineLoop_id=1):
    ...
    dpoints = {
        int(v.attrib["index"])
        + point_id: {
            "coords": [v.attrib["x"], v.attrib["y"], v.attrib["z"]],
            "lc": None,
        }
        for v in root[0][0]
    }
    ...
    for t in root[0][1]:
        ...
        data["v2"] = int(data["v2"])
        tlines = [
            (data["v0"] + point_id, data["v1"] + point_id),
...
def sort_lineLoop(geometry, lines_id):
    ...
    while len(sorted_lines) < nlines and i < 0:
        ...
        current = set(
            [
                geometry["lines"][sorted_lines[-1]][0],
                geometry["lines"][sorted_lines[-1]][1],
            ]
        )
        cond = True
        j = 0
        ...
        while cond and j < len(free_lines):
            free_line = set(
                [
                    geometry["lines"][free_lines[j]][0],
                    geometry["lines"][free_lines[j]][1],
                ]
            )
