...
def discretize_langloch(langloch, n=50):
    ...
    half_circle_0 = [
        m + r * np.array([np.cos(theta), np.sin(theta)]) for theta in angles_0
    ]
    ...
    new_m = m + v
    half_circle_1 = [
        new_m + r * np.array([np.cos(theta), np.sin(theta)]) for theta in angles_1
    ]
...
def stellar_polygon(N, r, center):
    ...
    return [
        center
        + r(i / N)
        * np.array([np.cos((i / N) * 2 * np.pi), np.sin((i / N) * 2 * np.pi)])
        for i in range(N)
    ]
...
def trigonometric_function(coeffients):
    ...
    return (
        lambda x: sum(
            [
                coeffients[k][0] * np.cos(k * 2 * np.pi * x)
                + coeffients[k][1] * np.sin(k * 2 * np.pi * x)
                for k in range(n)
            ]
        )
        + 4
