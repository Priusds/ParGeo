...
from bubbles.geometry import Rectangle, Stellar, Circle, NStar, RainDrop
...
def generate_topo():
    domain = Rectangle(midpoint=(0., 0.), width=2, height=1).to_polygon()
    ...
    n_stars = 100
    # random.seed(0)
    midpoints = [(2*random.random()-1, random.random()-0.5) for _ in range(n_stars)]
    ...
    n_rain = 200
    midpoints = [(2*random.random()-1, random.random()-0.5) for _ in range(n_rain)]
