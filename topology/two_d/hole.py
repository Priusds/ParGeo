...
class Rectangular(Hole):
    ...
    def to_dict(self):
        return {'type': self.type, 'midpoint' : self.midpoint, 'width' : self.width, 'height' : self.height}
...
def  discretize_ellipse_noisy(ellipse, refs, var):
    ...
    for i in range(n):
        theta = (i / n) * 2 * np.pi
        discrete_ellipse.append(rotate_counterclockwise(np.array([(rads[0] + noise[i]) * np.cos(theta), (rads[1] + noise[i]) * np.sin(theta)]),
                                                        alpha) + m)
    return [(p[0], p[1]) for p in discrete_ellipse]
...
def discetize_stellar_polygon(midpoint, radius, coefficients, refs):
    ...
    polygon = [(r*np.cos(omega) + midpoint[0], r*np.sin(omega) + midpoint[1]) for r, omega in zip(rad, X)]
