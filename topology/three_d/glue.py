    ...
    def add_bubble(self, bubble, accuracy=.5):
        ...
        if valid:
            ...
            if len(loc)==0:
                ...
                clip_bubble1, intersectionLoop1 = clip(clip_bubble0, clippingPlane1, EuclideanProjection, EasyClip)
                ...
                for i in range(2):
                    ...
                    line_id = abs(self.faces[loc[i]]["outerLoop"][localSide]["lines"][0])
                    lineLoop = intersectionLoops[i][1]
                    startPoint_Loop = clip_bubble1["lines"][lineLoop[0]][0] if lineLoop[0] > 0 else clip_bubble1["lines"][-lineLoop[0]][1]
                    
                    endPoint_Loop = clip_bubble1["lines"][lineLoop[-1]][1] if lineLoop[-1] > 0 else clip_bubble1["lines"][-lineLoop[-1]][0]
                    ...
                    l0,l1 = self.geometry["points"][l0]["coords"], self.geometry["points"][l1]["coords"]
                    if self.faces[loc[i]]["outerLoop"][localSide]["lines"][0] < 0:
                        temp = l1
                        l1 = l0
                        l0 = temp
                    if not compare(point0,point1,(l0,l1)):
                        self.faces[loc[i]]["outerLoop"][localSide]["lineLoops"].append([-l for l in reversed(intersectionLoops[i][1])])
                    else:
                        self.faces[loc[i]]["outerLoop"][localSide]["lineLoops"].append(intersectionLoops[i][1])

                self.geometry["points"] = {**self.geometry["points"], **clip_bubble1["points"]}
                self.geometry["lines"] = {**self.geometry["lines"], **clip_bubble1["lines"]}
                self.geometry["lineLoops"] = {**self.geometry["lineLoops"],**clip_bubble1["lineLoops"]}
                ...
                if len(self.faces[i]["outerLoop"][j]["lineLoops"]) == 0: # No intersection with Edges
                 ...
                 #   print("============ ",self.faces[i]["outerLoop"][j]["lineLoops"])
                    ...
                    if len(self.faces[i]["outerLoop"][j]["connections"]) == 0:  #len(self.faces[i]["outerLoop"][j]["connections"]) == 0:
                        ...
                        firstPoint = self.geometry["lines"][firstLine][0] if firstLine > 0 else self.geometry["lines"][-firstLine][1]
                        for lineLoop in self.faces[i]["outerLoop"][j]["lineLoops"]:
                            firstLine, lastLine = lineLoop[0], lineLoop[-1]
                            lastPoint = self.geometry["lines"][firstLine][0] if firstLine > 0 else self.geometry["lines"][-firstLine][1]
                            startEndPoints.append((firstPoint, lastPoint))
                            firstPoint = self.geometry["lines"][lastLine][1] if lastLine > 0 else self.geometry["lines"][-lastLine][0]
                        lastLine = self.faces[i]["outerLoop"][j]["lines"][0]
                        lastPoint = self.geometry["lines"][lastLine][1] if lastLine > 0 else self.geometry["lines"][-lastLine][0]
                        startEndPoints.append((firstPoint, lastPoint))
                        # Write new lines and add all lines to faceLineLoop
                        assert len(startEndPoints) == len(self.faces[i]["outerLoop"][j]["lineLoops"])+1
...
def sortLineLoops(line, geometry):
    start,end = geometry["lines"][line] if line > 0 else (geometry["lines"][-line][1],geometry["lines"][-line][0])
    startPoint = geometry["points"][start]["coords"]
    endPoint = geometry["points"][end]["coords"]
    def f(ll):
        firstLine = ll[0]
        firstPoint = geometry["lines"][firstLine][0] if firstLine > 0 else geometry["lines"][-firstLine][1]
