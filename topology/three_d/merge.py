...
def replaceLine(IdOld, newLines, geometry):
    ...
    if len(newLines)>2:
        ...
        lnewlines = [geometry["lines"][Id] if Id > 0 else geometry["lines"][-Id] for Id in newLines]
    else:
...
class Sanwich:
    def __init__(self, layers1, glue, layers2):
        ...
        self.nLayers = layers1.nLayers # Assume both layers are made of the same number of sublayers
        ...
        for i in range(2):
            ...
            layer["points"] = {shiftFactor+Id:layer["points"][Id] for Id in layer["points"].keys()}
            layer["lines"] = {shiftFactor+Id:tuple(pointId + shiftFactor for pointId in layer["lines"][Id]) for Id in layer["lines"].keys()}
            layer["lineLoops"] = {shiftFactor+Id: [lineId + shiftFactor  if lineId > 0 else lineId - shiftFactor for lineId in layer["lineLoops"][Id]] for Id in layer["lineLoops"].keys()}
            layer["surfaces"] = {shiftFactor+Id:[lineLoopId + shiftFactor for lineLoopId in layer["surfaces"][Id]] for Id in layer["surfaces"].keys()}
            layer["surfaceLoops"] = {shiftFactor+Id:[surfaceId + shiftFactor for surfaceId in layer["surfaceLoops"][Id]] for Id in layer["surfaceLoops"].keys()}
            layer["volumes"] = {shiftFactor+Id:[surfaceLoopId + shiftFactor for surfaceLoopId in layer["volumes"][Id]] for Id in layer["volumes"].keys()}
            sandwich = {
            ...
            self.merge[[444,44][i]]["points"]["left"] = [Id + shiftFactor for Id in self.merge[[444,44][i]]["points"]["left"]]
            self.merge[[333,33][i]]["points"]["left"] = [Id + shiftFactor for Id in self.merge[[333,33][i]]["points"]["left"]]
            self.merge[[444,44][i]]["points"]["right"] = [Id + shiftFactor for Id in self.merge[[444,44][i]]["points"]["right"]]
            self.merge[[333,33][i]]["points"]["right"] = [Id + shiftFactor for Id in self.merge[[333,33][i]]["points"]["right"]]

            self.merge[[444,44][i]]["lines"]["left"] = [Id + shiftFactor for Id in self.merge[[444,44][i]]["lines"]["left"]]
            self.merge[[333,33][i]]["lines"]["left"] = [Id + shiftFactor for Id in self.merge[[333,33][i]]["lines"]["left"]]
            self.merge[[444,44][i]]["lines"]["right"] = [Id + shiftFactor for Id in self.merge[[444,44][i]]["lines"]["right"]]
            self.merge[[333,33][i]]["lines"]["right"] = [Id + shiftFactor for Id in self.merge[[333,33][i]]["lines"]["right"]]
            self.merge[[444,44][i]]["lines"]["middle"] = [Id + shiftFactor for Id in self.merge[[444,44][i]]["lines"]["middle"]]
            self.merge[[333,33][i]]["lines"]["middle"] = [Id + shiftFactor for Id in self.merge[[333,33][i]]["lines"]["middle"]]

            self.merge[[333,33][i]]["lineLoops"] = [Id + shiftFactor for Id in self.merge[[333,33][i]]["lineLoops"]]
            self.merge[[444,44][i]]["lineLoops"] = [Id + shiftFactor for Id in self.merge[[444,44][i]]["lineLoops"]]

            self.merge[[333,33][i]]["surfaces"] = [Id + shiftFactor for Id in self.merge[[333,33][i]]["surfaces"]]
            self.merge[[444,44][i]]["surfaces"] = [Id + shiftFactor for Id in self.merge[[444,44][i]]["surfaces"]]
        ...
        if case == 1: # Glue and layers are aligned
            ...
            layers1PointsId = [Id + maxGlueId + maxLayer2Id for Id in layers1.merge[5]["points"]]

    
            
            for i in range(4):
                replacePoint(layers2PointsId[i], (gluePointIdsUp[i], sandwich["points"][gluePointIdsUp[i]]), sandwich)
                replacePoint(layers1PointsId[i], (gluePointIdsDown[i],sandwich["points"][gluePointIdsDown[i]]), sandwich)
            ...
            layers1LinesId = [Id+maxGlueId+maxLayer2Id for Id in layers1.merge[5]["lines"]]

            replaceLine(layers2LinesId[0], list(zip(*glue.merge[5]["mainLoopLayer"][1])),sandwich)
            replaceLine(layers2LinesId[1], list(zip(*glue.merge[5]["mainLoopLayer"][3])),sandwich)
            replaceLine(layers2LinesId[2], list(zip(*glue.merge[5]["mainLoopLayer"][2])),sandwich)
            replaceLine(layers2LinesId[3], list(zip(*glue.merge[5]["mainLoopLayer"][4])),sandwich)

            self.merge[333]["lines"]["middle"][0] = abs(glue.merge[5]["mainLoopLayer"][3][0][0])
            self.merge[444]["lines"]["middle"][0] = abs(glue.merge[5]["mainLoopLayer"][4][0][0])

            replaceLine(layers1LinesId[0], list(zip(*glue.merge[6]["mainLoopLayer"][1])),sandwich)
            replaceLine(layers1LinesId[1], list(zip(*glue.merge[6]["mainLoopLayer"][3])),sandwich)
            replaceLine(layers1LinesId[2], list(zip(*glue.merge[6]["mainLoopLayer"][2])),sandwich)
            replaceLine(layers1LinesId[3], list(zip(*glue.merge[6]["mainLoopLayer"][4])),sandwich)

            self.merge[33]["lines"]["middle"][-1] = abs(glue.merge[6]["mainLoopLayer"][3][0][0])
            self.merge[44]["lines"]["middle"][-1] = abs(glue.merge[6]["mainLoopLayer"][4][0][0])
            

            # LINELOOPS
            
            replaceLineLoop(layers2.merge[6]["lineLoop"]+maxGlueId,glue.merge[5]["loopId"],sandwich)
            
            replaceLineLoop(layers1.merge[5]["lineLoop"]+maxGlueId+maxLayer2Id,glue.merge[6]["loopId"],sandwich)
            
            # SURFACES
            replaceSurface(layers2.merge[6]["surface"]+maxGlueId,glue.merge[5]["surfaceId"],sandwich)
            replaceSurface(layers1.merge[5]["surface"]+maxGlueId+maxLayer2Id,glue.merge[6]["surfaceId"],sandwich)

            # Now we need to define new surfaces for the intersection between holes and facet
            freeSurfaceId = max(sandwich["surfaces"].keys())+1
            subSurfacesUp = list(range(freeSurfaceId, freeSurfaceId+len(glue.merge[5]["subLoops"])))
            for sl in glue.merge[5]["subLoops"]:
                sandwich["surfaces"][freeSurfaceId] = [sl]
                freeSurfaceId += 1

            subSurfacesDown = list(range(freeSurfaceId, freeSurfaceId+len(glue.merge[6]["subLoops"])))
            for sl in glue.merge[6]["subLoops"]:
                sandwich["surfaces"][freeSurfaceId] = [sl]
                freeSurfaceId += 1
            sandwich["surfaceLoops"][layers2.merge[6]["surfaceLoop"]+maxGlueId] = sandwich["surfaceLoops"][layers2.merge[6]["surfaceLoop"]+maxGlueId] + subSurfacesUp
            sandwich["surfaceLoops"][layers1.merge[5]["surfaceLoop"]+maxGlueId+maxLayer2Id] = sandwich["surfaceLoops"][layers1.merge[5]["surfaceLoop"]+maxGlueId+maxLayer2Id] + subSurfacesDown
            ...
            sandwich["lines"][freeLineId] = (4, [Id + maxGlueId for Id in layers2.merge[6]["points"]][0])
            freeLineId += 1

            lineRightFront = freeLineId
            sandwich["lines"][freeLineId] = ([Id + maxGlueId for Id in layers2.merge[6]["points"]][1],3)
            freeLineId += 1

            lineRightBack = freeLineId
            
            sandwich["lines"][freeLineId] = ([Id + maxGlueId for Id in layers2.merge[6]["points"]][2],7)
            freeLineId += 1

            lineLeftBack = freeLineId
            sandwich["lines"][freeLineId] = (8,[Id + maxGlueId for Id in layers2.merge[6]["points"]][3])
            freeLineId += 1

            replaceLine([Id + maxGlueId for Id in layers2.merge[6]["lines"]][0],[-lineLeftFront,-3,-lineRightFront],sandwich)
            replaceLine([Id + maxGlueId for Id in layers2.merge[6]["lines"]][2],[lineRightBack,7,lineLeftBack],sandwich)
            ...
            sandwich["surfaceLoops"][layers2.merge[6]["surfaceLoop"]+maxGlueId].remove(maxGlueId +  layers2.merge[6]["surface"])
            ...
            # make SurfaceLoop
            sandwich["surfaceLoops"][layers2.merge[6]["surfaceLoop"]+maxGlueId] = sandwich["surfaceLoops"][layers2.merge[6]["surfaceLoop"]+maxGlueId] + [surfaceLeft, surfaceRight] + subSurfacesUp + [glue.merge[5]["surfaceId"]]
            ...
            lineLeftFront = freeLineId
            sandwich["lines"][freeLineId] = (1, [Id + maxGlueId + maxLayer2Id for Id in layers1.merge[5]["points"]][0])
            freeLineId += 1

            lineRightFront = freeLineId
            sandwich["lines"][freeLineId] = ([Id + maxGlueId  + maxLayer2Id for Id in layers1.merge[5]["points"]][1],2)
            freeLineId += 1

            lineRightBack = freeLineId
            
            sandwich["lines"][freeLineId] = ([Id + maxGlueId + maxLayer2Id for Id in layers1.merge[5]["points"]][2],6)
            freeLineId += 1

            lineLeftBack = freeLineId
            sandwich["lines"][freeLineId] = (5,[Id + maxGlueId + maxLayer2Id for Id in layers1.merge[5]["points"]][3])
            freeLineId += 1

            replaceLine([Id + maxGlueId+maxLayer2Id for Id in layers1.merge[5]["lines"]][0],[-lineLeftFront,1,-lineRightFront],sandwich)
            replaceLine([Id + maxGlueId+maxLayer2Id for Id in layers1.merge[5]["lines"]][2],[lineRightBack,-5,lineLeftBack],sandwich)
            
            # Remove lineLoop
            sandwich["lineLoops"].pop(maxGlueId +  layers1.merge[5]["lineLoop"] + maxLayer2Id)
            
            # Remove surface
            sandwich["surfaces"].pop(maxGlueId +  layers1.merge[5]["surface"]+ maxLayer2Id)
            sandwich["surfaceLoops"][layers1.merge[5]["surfaceLoop"]+maxGlueId+maxLayer2Id].remove(maxGlueId +  layers1.merge[5]["surface"]+ maxLayer2Id)
            ...
            # make SurfaceLoop
            sandwich["surfaceLoops"][layers1.merge[5]["surfaceLoop"]+maxGlueId+maxLayer2Id] = sandwich["surfaceLoops"][layers1.merge[5]["surfaceLoop"]+maxGlueId+maxLayer2Id] + [surfaceLeft, surfaceRight] + subSurfacesDown + [glue.merge[6]["surfaceId"]]
    ...
    def shift(self, shiftFactor):
        shiftedPoints = {Id + shiftFactor:self.geometry["points"][Id] for Id in self.geometry["points"]}
        shiftedLines = {Id + shiftFactor:tuple(pId + shiftFactor for pId in self.geometry["lines"][Id]) for Id in self.geometry["lines"]}
        shiftedLineLoops = {Id + shiftFactor:[lId + shiftFactor if lId > 0 else lId - shiftFactor for lId in self.geometry["lineLoops"][Id]] for Id in self.geometry["lineLoops"]}
        shiftedSurfaces = {Id + shiftFactor:[llId + shiftFactor for llId in self.geometry["surfaces"][Id]] for Id in self.geometry["surfaces"]}
        shiftedSurfaceLoops = {Id + shiftFactor:[sId + shiftFactor for sId in self.geometry["surfaceLoops"][Id]] for Id in self.geometry["surfaceLoops"]}
        shiftedVolumes = {Id + shiftFactor:[ssId + shiftFactor for ssId in self.geometry["volumes"][Id]] for Id in self.geometry["volumes"]}
        ...
        self.merge[33]["points"]["left"] = [Id + shiftFactor for Id in self.merge[33]["points"]["left"]]
        self.merge[333]["points"]["left"] = [Id + shiftFactor for Id in self.merge[333]["points"]["left"]]
        self.merge[44]["points"]["left"] = [Id + shiftFactor for Id in self.merge[44]["points"]["left"]]
        self.merge[444]["points"]["left"] = [Id + shiftFactor for Id in self.merge[444]["points"]["left"]]

        self.merge[33]["points"]["right"] = [Id + shiftFactor for Id in self.merge[33]["points"]["right"]]
        self.merge[333]["points"]["right"] = [Id + shiftFactor for Id in self.merge[333]["points"]["right"]]
        self.merge[44]["points"]["right"] = [Id + shiftFactor for Id in self.merge[44]["points"]["right"]]
        self.merge[444]["points"]["right"] = [Id + shiftFactor for Id in self.merge[444]["points"]["right"]]

        self.merge[33]["lines"]["right"] = [Id + shiftFactor for Id in self.merge[33]["lines"]["right"]]
        self.merge[333]["lines"]["right"] = [Id + shiftFactor for Id in self.merge[333]["lines"]["right"]]
        self.merge[44]["lines"]["right"] = [Id + shiftFactor for Id in self.merge[44]["lines"]["right"]]
        self.merge[444]["lines"]["right"] = [Id + shiftFactor for Id in self.merge[444]["lines"]["right"]]

        self.merge[33]["lines"]["left"] = [Id + shiftFactor for Id in self.merge[33]["lines"]["left"]]
        self.merge[333]["lines"]["left"] = [Id + shiftFactor for Id in self.merge[333]["lines"]["left"]]
        self.merge[44]["lines"]["left"] = [Id + shiftFactor for Id in self.merge[44]["lines"]["left"]]
        self.merge[444]["lines"]["left"] = [Id + shiftFactor for Id in self.merge[444]["lines"]["left"]]

        self.merge[33]["lines"]["middle"] = [Id + shiftFactor for Id in self.merge[33]["lines"]["middle"]]
        self.merge[333]["lines"]["middle"] = [Id + shiftFactor for Id in self.merge[333]["lines"]["middle"]]
        self.merge[44]["lines"]["middle"] = [Id + shiftFactor for Id in self.merge[44]["lines"]["middle"]]
        self.merge[444]["lines"]["middle"] = [Id + shiftFactor for Id in self.merge[444]["lines"]["middle"]]

        self.merge[33]["lineLoops"] = [Id + shiftFactor for Id in self.merge[33]["lineLoops"]]
        self.merge[333]["lineLoops"] = [Id + shiftFactor for Id in self.merge[333]["lineLoops"]]
        self.merge[44]["lineLoops"] = [Id + shiftFactor for Id in self.merge[44]["lineLoops"]]
        self.merge[444]["lineLoops"] = [Id + shiftFactor for Id in self.merge[444]["lineLoops"]]

        self.merge[33]["surfaces"] = [Id + shiftFactor for Id in self.merge[33]["surfaces"]]
        self.merge[333]["surfaces"] = [Id + shiftFactor for Id in self.merge[333]["surfaces"]]
        self.merge[44]["surfaces"] = [Id + shiftFactor for Id in self.merge[44]["surfaces"]]
        self.merge[444]["surfaces"] = [Id + shiftFactor for Id in self.merge[444]["surfaces"]]
    ...
    def __init__(self, s1, s2, s3, s4):
        ...
        for j in range(4):
            ...
            for i in range(nLayers+1):
                self.geometry["lines"][freeLineId] = (sandwichUp["points"]["left"][i],
                                                            sandwichDown["points"]["right"][i])
                freeLineId += 1
            
            linesBack = range(freeLineId, freeLineId+nLayers+1)
            for i in range(self.nLayers+1):
                self.geometry["lines"][freeLineId] = (sandwichUp["points"]["right"][i],
